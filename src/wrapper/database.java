...
public class database
{
  ...
  public static void openBusDatabase()
  {
    try
    {
      busDatabase = new database("storeinfo",
              "root",
              "",
  ...
  */
  ...
  public Object SqlQuery(String SqlCmd,String field_name)
  {
       try
    {
       Statement s = connection.createStatement();
  ...
  public int find_id_new(String table, String field, Object value)
  {
    select("items_id", table, field + " = " + value_string(value), "");
    if (move_first()) return (Integer)get_field("items_id");
    else return 0;
  }
  ...
  public String[] select_dates(String id_field, String source, String order)
  {
    SimpleDateFormat df = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
      ...
      String reportDate = df.format(id_field);
    int  count= record_count(id_field, source, "");
    ...
    { 
      ...
      results[i] = (String)get_field(id_field.replaceFirst("Distinct ", ""));
    }
    return results;
  }

  public int[] select_ids_same(String id_field, String source, String order)
  {
    int    count    = record_count(id_field, source, "");
    int[]  results  = new int[count];
    select(id_field, source, "", order);
    for (int i = 0; i < count && move_next(); i = i + 1)
      results[i] = (Integer)get_field(id_field.replaceFirst("", ""));
    return results;
  }
...
public int[] select_ids_same(String id_field, String source, String field, Object value, String order)
  {
    int    count    = record_count(id_field, source, field + " = " + value_string(value));
    int[]  results  = new int[count];
    select(id_field, source, field + " = " + value_string(value), order);
  ...
  public Double get_float(String table, int id, String field_name)
  {
    select(field_name, table, table + "_id = " + id, "");
    if (move_first())
      return (Double)get_field(field_name);
    else return 0.0;
  }
