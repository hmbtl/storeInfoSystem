 ...
 */
  ...
  {
    return database.busDatabase.select_ids("users_id", "users", "users_id");
  }
 ...
 */
  ...
  {
    if (user == 0) throw new InvalidQueryException("Nonexistent driver");
      return database.busDatabase.get_string("users", user, "name");
  }

   public static String getUserName(int user)
  {
    if (user == 0) throw new InvalidQueryException("Nonexistent driver");
      return database.busDatabase.get_string("users", user, "username");
  }
   public static String getUserPassword(int user)
  {
    if (user == 0) throw new InvalidQueryException("Nonexistent driver");
      return database.busDatabase.get_string("users", user, "password");
  }
 ...
 */
  ...
  //increase hours this week
  public static void increaseHoursThisWeek(int driver, int value)
  {
    if (driver == 0) 
      throw new InvalidQueryException("Nonexistent driver");
    int currentHours = getHoursThisWeek(driver);
    currentHours += value;
    database.busDatabase.set_value("driver", driver, "hours_this_week", currentHours);
  }//increaseHoursThisWeek
 ...
 */
  public static void setAvailable(int driver, Date date, boolean available)
  {
    ...
    if (available && !isAvailable(driver, date))
      ...
      database.busDatabase.new_record("driver_availability", new Object[][]{{"available", false}, {"day", date}, 
                                      {"driver", driver}});
  }//setAvailable
